...
import { Star, Moon, Calendar, MapPin, Github, Coffee, ArrowUpDown, Cloud, Filter } from 'lucide-react';
...
function App() {
  ...
  const [skyQualityFilter, setSkyQualityFilter] = useState<number[]>([1, 2, 3, 4, 5, 6, 7, 8, 9]); // All Bortle scales by default

  // Sort and filter dark sky zones based on selected criteria and limit display
  const sortedDarkSkyZones = useMemo(() => {
    if (darkSkyZones.length === 0) return [];
    
    // First filter by sky quality (Bortle scale) if sorting by distance
    let filtered = darkSkyZones;
    if (sortBy === 'distance') {
      filtered = darkSkyZones.filter(zone => skyQualityFilter.includes(zone.bortle_scale));
    }
  ...
  }, [darkSkyZones, sortBy, displayLimit, skyQualityFilter]);

  // Helper function to get filtered zone count
  const getFilteredZoneCount = () => {
    if (sortBy === 'distance') {
      return darkSkyZones.filter(zone => skyQualityFilter.includes(zone.bortle_scale)).length;
    }
    return darkSkyZones.length;
  };
              ...
              <div className="mb-8">
                ...
                {activeTab === 'zones' && darkSkyZones.length > 0 && (
                  <div className="animate-fade-in">
                    <div className="flex flex-col lg:flex-row lg:items-center justify-between mb-6 gap-4">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-sm text-gray-400 bg-white/10 px-2 py-1 rounded-full">
                          Showing {sortedDarkSkyZones.length} of {getFilteredZoneCount()}
                          {sortBy === 'distance' && skyQualityFilter.length < 9 && ' (filtered)'}
                        </span>
                      </div>
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
                        ...
                        {/* Sky Quality Filter - Only show when sorting by distance */}
                        {sortBy === 'distance' && (
                          <div className="flex items-center space-x-2">
                            <Filter className="text-gray-400" size={16} />
                            <select
                              value={skyQualityFilter.length === 9 ? 'all' : 
                                    skyQualityFilter.length === 4 && skyQualityFilter.every(s => s <= 4) ? 'dark' :
                                    skyQualityFilter.length === 2 && skyQualityFilter.every(s => s <= 2) ? 'pristine' :
                                    skyQualityFilter.length === 5 && skyQualityFilter.every(s => s <= 5) ? 'rural' :
                                    'all'}
                              onChange={(e) => {
                                const value = e.target.value;
                                if (value === 'all') {
                                  setSkyQualityFilter([1, 2, 3, 4, 5, 6, 7, 8, 9]);
                                } else if (value === 'dark') {
                                  setSkyQualityFilter([1, 2, 3, 4]);
                                } else if (value === 'pristine') {
                                  setSkyQualityFilter([1, 2]);
                                } else if (value === 'rural') {
                                  setSkyQualityFilter([1, 2, 3, 4, 5]);
                                }
                                setDisplayLimit(5);
                                setPreviousDisplayLimit(0);
                              }}
                            ...
                            >
                              ...
                              <option value="dark" className="bg-gray-800">Dark (1-4)</option>
                              <option value="rural" className="bg-gray-800">Rural+ (1-5)</option>
                            </select>
                          </div>
                        )}
                      </div>
                    ...
                    {displayLimit < getFilteredZoneCount() && (
